...
using SharpMap.Demo.FormatConverter.PostGis.Properties;
...
namespace SharpMap.Demo.FormatConverter.PostGis
{
    ...
    public class ConfigurePostGisProvider : IConfigureFeatureSource, IConfigureFeatureTarget
    {
        ...
        public void PostImport()
        {
            ...
            {
                if (_sourceProvider.ConnectionId != Settings.Default.SourceConnectionString)
                {
                    Console.WriteLine(
                        "\nDo you want to save the source provider connection string for future use (Y/any other key)?");
                    if (Console.ReadKey(true).Key == ConsoleKey.Y)
                    {
                        Settings.Default.SourceConnectionString = _sourceProvider.ConnectionId;
                        settingChanged = true;
                    }
                }
                if (_sourceProvider.OriginalSrid != _sourceProvider.Srid)
                {
                    ...
                    if (Console.ReadKey(true).Key == ConsoleKey.Y)
                    {
                        Settings.Default.TargetSrid = _sourceProvider.Srid;
                        settingChanged = true;
                    }
                }
            }
            ...
            {
                if (_targetProvider.ConnectionId != Settings.Default.TargetConnectionString)
                {
                    Console.WriteLine(
                        "\nDo you want to save the target provider connection string for future use (Y/any other key)");
                    if (Console.ReadKey(true).Key == ConsoleKey.Y)
                    {
                        Settings.Default.TargetConnectionString = _targetProvider.ConnectionId;
                        settingChanged = true;
                    }
                }
                //Console.WriteLine( "\nCleaning up: VACUUM" );
        ...
        private ICoordinateTransformation SetCoordinateTransfromation(ICoordinateSystem spatialReference)
        {
            if (spatialReference != null)
            {
                ...
                if (string.IsNullOrEmpty(targetCS)) targetCS = Settings.Default.TargetSrid;
