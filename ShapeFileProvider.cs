...
namespace SharpMap.Data.Providers.ShapeFile
{
    ...
    public class ShapeFileProvider : FeatureProviderBase, IWritableFeatureProvider<UInt32>
    {
        ...
        public ShapeFileProvider(String filename, IGeometryFactory geoFactory)
            : this(filename, geoFactory, null, false, false)
        {
        }
        ...
        public ShapeFileProvider(String filename,
            ...
            : this(filename, geoFactory, coordSysFactory, false, false)
        {
        }
        ...
        public ShapeFileProvider(String filename,
                                 IGeometryFactory geoFactory,
                                 ICoordinateSystemFactory coordSysFactory,
                                 Boolean fileBasedIndex, bool writable)
        {
            _filename = filename;
            ...
            using (FileStream fs = new FileStream(filename, FileMode.Open, FileAccess.Read, IsReadonly ? FileShare.ReadWrite : FileShare.Read))
            ...
            if (HasDbf)
            {
                _dbaseFile = new DbaseFile(DbfFilename, geoFactory, IsReadonly);
            }
        }
                                               ...
                                               ICoordinateSystemFactory coordinateSystemFactory)
        {
            ...
            return new ShapeFileProvider(shapeFile, geoFactory, coordinateSystemFactory, false, true);
        }
        ...
        public void Open(Boolean exclusive)
        {
            ...
            try
            {
                ...
                _shapeFileStream = new FileStream(Filename,
                                                  ...
                                                  exclusive ? FileShare.None : IsReadonly ? FileShare.ReadWrite : FileShare.Read,
                                                  4096,
                ...
                if (HasDbf)
                {
                    _dbaseFile = new DbaseFile(DbfFilename, GeometryFactory, IsReadonly);
        ...
        private ICoordinate readCoordinate(StreamOffsetUtility offsetUtility, ShapeType shpType, int numShapeParts, int numShapePoints, int pointIndex)
        {
            ...
            if (!offset.Z.HasValue && !offset.M.HasValue)
            {
                switch (ForceCoordinateOptions)
                {
                    ...
                    case ForceCoordinateOptions.Force2D:
                        {
                            coordinate = _coordFactory.Create(values.X, values.Y);
                            break;
                        }
                    case ForceCoordinateOptions.Force2DM:
                        {
        ...
        private IGeometry readPolyLine(StreamOffsetUtility offsetUtility, ShapeType shpType)
        {
            ...
            for (Int32 lineId = 0; lineId < parts; lineId++)
            {
                ...
                readSegments(offsetUtility, shpType, parts, points, lineId, segments, coordinates);
        ...
        private IGeometry readPolygon(StreamOffsetUtility offsetUtility, ShapeType shpType)
        {
            ...
            for (Int32 ringId = 0; ringId < parts; ringId++)
            {
                ...
                readSegments(offsetUtility, shpType, parts, points, ringId, segments, coordinates);
