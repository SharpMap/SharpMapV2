...
namespace ProjNet.UnitTests
{
	...
	public class CoordinateTransformTests
	{
		...
		public void TestAlbersProjection()
		{
			...
			CoordinateSystemFactory<Coordinate2D> cFac = new CoordinateSystemFactory<Coordinate2D>(cf, gf);

			IEllipsoid ellipsoid = cFac.CreateFlattenedSphere(6378206.4, 294.9786982138982, LinearUnit.USSurveyFoot, "Clarke 1866");
			...
			IGeographicCoordinateSystem<Coordinate2D> gcs = cFac.CreateGeographicCoordinateSystem(extents, AngularUnit.Degrees, datum,
				PrimeMeridian.Greenwich, new AxisInfo(AxisOrientation.East, "Lon"),
				new AxisInfo(AxisOrientation.North, "Lat"), "Clarke 1866");
			...
			IProjection projection = cFac.CreateProjection("albers", parameters, "Albers Conical Equal Area");

			IProjectedCoordinateSystem<Coordinate2D> coordsys = cFac.CreateProjectedCoordinateSystem(gcs, projection, LinearUnit.Meter, new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "Albers Conical Equal Area");

			ICoordinateTransformation trans = 
				new CoordinateTransformationFactory<Coordinate2D>(cf, gf, new MatrixFactory())
				.CreateFromCoordinateSystems(gcs, coordsys as ICoordinateSystem<Coordinate2D>);
		...
		public void TestAlbersProjectionFeet()
		{
			...
			IProjectedCoordinateSystem<Coordinate2D> coordsys = cFac.CreateProjectedCoordinateSystem(gcs, projection, LinearUnit.Foot, new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "Albers Conical Equal Area");
			...
			ICoordinate expected = gf.CreatePoint2D(1885472.7 / LinearUnit.Foot.MetersPerUnit, 1535925 / LinearUnit.Foot.MetersPerUnit).Coordinate;
		...
		public void TestMercator_1SP_Projection()
		{
			...
			IProjection projection = cFac.CreateProjection("Mercator_1SP", parameters, "Mercator_1SP");

			IProjectedCoordinateSystem<Coordinate2D> coordsys = cFac.CreateProjectedCoordinateSystem(gcs, projection, LinearUnit.Meter, new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "Makassar / NEIEZ");
		...
		public void TestMercator_1SP_Projection_Feet()
		{
			...
			IProjectedCoordinateSystem<Coordinate2D> coordsys = cFac.CreateProjectedCoordinateSystem(gcs, projection, LinearUnit.Foot, new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "Makassar / NEIEZ");
			...
			ICoordinate expected = gf.CreatePoint2D(5009726.58 / LinearUnit.Foot.MetersPerUnit, 569150.82 / LinearUnit.Foot.MetersPerUnit).Coordinate;
			Assert.IsTrue(ToleranceLessThan(pUtm, expected, 0.02), String.Format("Mercator_1SP forward transformation outside tolerance, Expected [{0},{1}], got [{2},{3}]", expected[0],expected[1], pUtm[0],pUtm[1]));
			Assert.IsTrue(ToleranceLessThan(pGeo, pGeo2, 0.0000001), String.Format("Mercator_1SP reverse transformation outside tolerance, Expected [{0},{1}], got [{2},{3}]", pGeo[0], pGeo[1], pGeo2[0], pGeo2[1]));
		}
		[Test]
		public void TestMercator_2SP_Projection()
		{
			...
			IEllipsoid ellipsoid = cFac.CreateFlattenedSphere(6378245.0, 298.3, LinearUnit.Meter, "Krassowski 1940");

			IHorizontalDatum datum = cFac.CreateHorizontalDatum(DatumType.HorizontalGeocentric, ellipsoid, null, "Krassowski 1940");
			IExtents<Coordinate2D> extents = gf.CreateExtents();
			IGeographicCoordinateSystem<Coordinate2D> gcs = cFac.CreateGeographicCoordinateSystem(extents, AngularUnit.Degrees, datum,
				PrimeMeridian.Greenwich, new AxisInfo(AxisOrientation.East, "Lon"),
				new AxisInfo(AxisOrientation.North, "Lat"), "Krassowski 1940");
			List<ProjectionParameter> parameters = new List<ProjectionParameter>(5);
			parameters.Add(new ProjectionParameter("latitude_of_origin", 42));
			parameters.Add(new ProjectionParameter("central_meridian", 51));
			parameters.Add(new ProjectionParameter("false_easting", 0));
			parameters.Add(new ProjectionParameter("false_northing", 0));
			IProjection projection = cFac.CreateProjection("Mercator_2SP", parameters, "Mercator_2SP");

			IProjectedCoordinateSystem<Coordinate2D> coordsys = cFac.CreateProjectedCoordinateSystem(gcs, projection, LinearUnit.Meter, new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "Pulkovo 1942 / Mercator Caspian Sea");
		...
		public void TestTransverseMercator_Projection()
		{
			...
			IProjection projection = cFac.CreateProjection("Transverse_Mercator", parameters, "Transverse Mercator");

			IProjectedCoordinateSystem<Coordinate2D> coordsys = cFac.CreateProjectedCoordinateSystem(gcs, projection, LinearUnit.Meter, new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "OSGB 1936 / British National Grid");
		...
		public void TestLambertConicConformal2SP_Projection()
		{
			...
			IEllipsoid ellipsoid = cFac.CreateFlattenedSphere(20925832.16, 294.97470, LinearUnit.USSurveyFoot, "Clarke 1866");
			...
			parameters.Add(new ProjectionParameter("false_northing", 0));
			IProjection projection = cFac.CreateProjection("lambert_conformal_conic_2sp", parameters, "Lambert Conic Conformal (2SP)");

			IProjectedCoordinateSystem<Coordinate2D> coordsys = cFac.CreateProjectedCoordinateSystem(gcs, projection, LinearUnit.USSurveyFoot, new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "NAD27 / Texas South Central");
			...
			ICoordinate expected = gf.CreatePoint2D(2963503.91 / LinearUnit.USSurveyFoot.MetersPerUnit, 254759.80 / LinearUnit.USSurveyFoot.MetersPerUnit).Coordinate;
		...
		public void TestGeocentric()
		{
			...
			IExtents<Coordinate2D> extents = gf.CreateExtents();
			IGeographicCoordinateSystem<Coordinate2D> gcs = cFac.CreateGeographicCoordinateSystem(extents, AngularUnit.Degrees, HorizontalDatum.Etrf89, PrimeMeridian.Greenwich,
				new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "ETRF89 Geographic");
			IGeocentricCoordinateSystem gcenCs = cFac.CreateGeocentricCoordinateSystem(extents, HorizontalDatum.Etrf89, LinearUnit.Meter, PrimeMeridian.Greenwich, "ETRF89 Geographic");
			CoordinateTransformationFactory<Coordinate2D> gtFac = new CoordinateTransformationFactory<Coordinate2D>(cf, gf, new MatrixFactory());
			ICoordinateTransformation ct = gtFac.CreateFromCoordinateSystems(gcs, gcenCs as ICoordinateSystem<Coordinate2D>);
			IPoint2D pExpected = gf.CreatePoint2D(2 + 7.0 / 60 + 46.38 / 3600, 53 + 48.0 / 60 + 33.82/3600); // Point.FromDMS(2, 7, 46.38, 53, 48, 33.82);
		...
		public void TestDatumTransform()
		{
			...
			IGeographicCoordinateSystem<Coordinate2D> gcsWGS72 =
				cFac.CreateGeographicCoordinateSystem(extents, AngularUnit.Degrees, wgs72, PrimeMeridian.Greenwich,
				new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "WGS72 Geographic");

			IGeographicCoordinateSystem<Coordinate2D> gcsWGS84 =
				cFac.CreateGeographicCoordinateSystem(extents, AngularUnit.Degrees, HorizontalDatum.Wgs84, PrimeMeridian.Greenwich,
				new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "WGS84 Geographic");

			IGeographicCoordinateSystem<Coordinate2D> gcsED50 =
				cFac.CreateGeographicCoordinateSystem(extents, AngularUnit.Degrees, ed50, PrimeMeridian.Greenwich,
				new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "ED50 Geographic");

			//Define geocentric coordinate systems
			IGeocentricCoordinateSystem<Coordinate2D> gcenCsWGS72 =
				cFac.CreateGeocentricCoordinateSystem(extents, wgs72, LinearUnit.Meter, PrimeMeridian.Greenwich, "WGS72 Geocentric");
			IGeocentricCoordinateSystem<Coordinate2D> gcenCsWGS84 =
				cFac.CreateGeocentricCoordinateSystem(extents, HorizontalDatum.Wgs84, LinearUnit.Meter, PrimeMeridian.Greenwich, "WGS84 Geocentric");
			IGeocentricCoordinateSystem<Coordinate2D> gcenCsED50 =
				cFac.CreateGeocentricCoordinateSystem(extents, ed50, LinearUnit.Meter, PrimeMeridian.Greenwich, "ED50 Geocentric");
			...
			parameters.Add(new ProjectionParameter("false_northing", 0));
			IProjection projection = cFac.CreateProjection("Transverse_Mercator", parameters, "Transverse Mercator");
			IProjectedCoordinateSystem<Coordinate2D> utmED50 = cFac.CreateProjectedCoordinateSystem(gcsED50, projection, LinearUnit.Meter, new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "ED50 UTM Zone 32N");
			IProjectedCoordinateSystem<Coordinate2D> utmWGS84 = cFac.CreateProjectedCoordinateSystem(gcsWGS84, projection, LinearUnit.Meter, new AxisInfo(AxisOrientation.East, "East"), new AxisInfo(AxisOrientation.North, "North"), "WGS84 UTM Zone 32N");

			//Set up coordinate transformations
			CoordinateTransformationFactory<Coordinate2D> ctFac = new CoordinateTransformationFactory<Coordinate2D>(cf, gf, new MatrixFactory());
			...
			ICoordinate pGeoCenWGS72 = gf.CreatePoint3D(3657660.66, 255768.55, 5201382.11).Coordinate;
			...
			Assert.IsTrue(ToleranceLessThan(gf.CreatePoint3D(3657660.78, 255778.43, 5201387.75).Coordinate, pGeoCenWGS84, 0.01));
			...
			Assert.IsTrue(ToleranceLessThan(gf.CreatePoint2D(599928.6, 6099790.2).Coordinate, pUTMWGS84, 0.1));
			//Perform reverse
			...
			Assert.IsTrue(ToleranceLessThan(gf.CreatePoint2D(600000, 6100000).Coordinate, pUTMED50, 0.1));
        ...
        {
			ICoordinateSystem<Coordinate2D> nadUTM = SRIDReader.GetCSbyID(2868); //UTM Arizona Central State Plane using Feet as units
	...
	internal class FakeMatrixFactory : IMatrixFactory<DoubleComponent>
	{
