...
namespace AGG.PixelFormat
{
    ...
    public interface IBlender
    {
        ...
        unsafe void BlendPix(byte* p, uint cr, uint cg, uint cb, uint alpha);
        unsafe void BlendPix(byte* p, uint cr, uint cg, uint cb, uint alpha, uint cover);
    }
    ...
    public sealed class BlenderBGRA : BlenderBaseBGRA, IBlender
    {
        ...
        unsafe public void BlendPix(byte* p, uint cr, uint cg, uint cb, uint alpha)
        {
            unchecked
            {
                ...
                p[(int)Order.R] = (byte)(((uint)(cr - r) * alpha + (r << (int)ColorConstants.BaseShift)) >> (int)ColorConstants.BaseShift);
                ...
                p[(int)Order.A] = (byte)((alpha + a) - ((alpha * a + BaseMask) >> (int)ColorConstants.BaseShift));
            }
        }
    };
    ...
    public sealed class BlenderGammaBGRA : BlenderBaseBGRA, IBlender
    {
        ...
        public BlenderGammaBGRA(GammaLut g)
        {
            m_gamma = g;
        }
    ...
    public sealed class BlenderPreMultBGRA : BlenderBaseBGRA, IBlender
    {
        unsafe public void BlendPix(byte* p, uint cr, uint cg, uint cb, uint alpha, uint cover)
        {
            ...
            unchecked
            {
                ...
                p[(int)Order.A] = (byte)(BaseMask - ((OneOverAlpha * (BaseMask - p[(int)Order.A])) >> (int)ColorConstants.BaseShift));
            }
        }
    ...
    public sealed class BlenderPreMultClampedBGRA : BlenderBaseBGRA, IBlender
    {
        unsafe public void BlendPix(byte* p, uint cr, uint cg, uint cb, uint alpha, uint cover)
        {
            ...
            p[(int)Order.A] = (byte)System.Math.Min((BaseMask - ((OneOverAlpha * (BaseMask - p[(int)Order.A])) >> (int)ColorConstants.BaseShift)), BaseMask);
        }
        ...
        {
            uint OneOverAlpha = BaseMask - alpha;
            p[(int)Order.R] = (byte)System.Math.Min((((p[(int)Order.R] * OneOverAlpha) >> (int)ColorConstants.BaseShift) + cr), BaseMask);
            p[(int)Order.G] = (byte)System.Math.Min((((p[(int)Order.G] * OneOverAlpha) >> (int)ColorConstants.BaseShift) + cr), BaseMask);
            p[(int)Order.B] = (byte)System.Math.Min((((p[(int)Order.B] * OneOverAlpha) >> (int)ColorConstants.BaseShift) + cr), BaseMask);
            p[(int)Order.A] = (byte)System.Math.Min((BaseMask - ((OneOverAlpha * (BaseMask - p[(int)Order.A])) >> (int)ColorConstants.BaseShift)), BaseMask);
        }
...
*/
    ...
    public sealed class FormatRGBA : IPixelFormat
    {
        ...
        {
            value_type* p = (value_type*)m_rbuf->row_ptr(y);
            if(p)
            {
                p += x << 2;
                return p;
        ...
        public void BlendHLine(int x1, int y, int x2, RGBA_Bytes c, byte cover)
        {
            if (c.A != 0)
            {
                ...
                unsafe
                {
                    ...
                    {
                        uint v;
                        ((byte*)&v)[OrderR] = (byte)c.R;
                        ((byte*)&v)[OrderG] = (byte)c.G;
                        ((byte*)&v)[OrderB] = (byte)c.B;
                        ((byte*)&v)[OrderA] = (byte)c.A;
                        do
                        {
                            *(uint*)p = v;
                            p += 4;
        ...
        //--------------------------------------------------------------------
        public unsafe void CopyColorHSpan(int x, int y, uint len, RGBA_Bytes* colors)
        {
            byte* p = m_rbuf.GetPixelPointer(y) + (x << 2);
            do
            {
        ...
        unsafe public void BlendColorHSpan(int x, int y, uint len, RGBA_Bytes* colors, byte* covers, byte cover)
        {
            ...
            if (covers != null)
            {
                do
                {
                    CopyOrBlendBGRAWrapper.CopyOrBlendPix(m_Blender, p, colors->R, colors->G, colors->B, colors->A, *covers++);
                    p += 4;
                    ++colors;
                }
                while (--len != 0);
            ...
            else
            {
                if (cover == 255)
                {
                    do
                    {
                        CopyOrBlendBGRAWrapper.CopyOrBlendPix(m_Blender, p, colors->R_Byte, colors->G_Byte, colors->B_Byte, colors->A_Byte);
                        p += 4;
                        ++colors;
                    }
                    while (--len != 0);
                ...
                else
                {
                    do
                    {
                        CopyOrBlendBGRAWrapper.CopyOrBlendPix(m_Blender, p, colors->R_Byte, colors->G_Byte, colors->B_Byte, colors->A_Byte, cover);
                        p += 4;
                        ++colors;
                    }
                    while (--len != 0);
