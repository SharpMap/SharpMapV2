...
namespace AGG
{
    public static class MathUtil
    {
        ...
        public const double VertexDistEpsilon = 1e-14;
        ...
        public static T CrossProduct<T>(T x1, T y1,
                                        T x2, T y2,
                                        T x, T y)
            where T : IEquatable<T>, IComparable<T>, IComputable<T>, IConvertible, IFormattable
        {
            return x.Subtract(x2).Multiply(y2.Subtract(y1)).Subtract(y.Subtract(y2)).Multiply((x2.Subtract(x1)));
        }
        ...
        public static bool PointInTriangle<T>(
                                              T x1, T y1,
                                              T x2, T y2,
                                              T x3, T y3,
                                              T x, T y)
        ...
        {
            bool cp1 = CrossProduct(x1, y1, x2, y2, x, y).LessThan(0.0);
            bool cp2 = CrossProduct(x2, y2, x3, y3, x, y).LessThan(0.0);
            bool cp3 = CrossProduct(x3, y3, x1, y1, x, y).LessThan(0.0);
            return cp1.Equals(cp2) && cp2.Equals(cp3) && cp3.Equals(cp1);
        }

        //-----------------------------------------------------------calc_distance
        public static T CalcDistance<T>(T x1, T y1, T x2, T y2)
              where T : IEquatable<T>, IComparable<T>, IComputable<T>, IConvertible, IFormattable, ICommonNumericalOperations<T>, ITrigonometricOperations<T>
        {
            //T dx = x2.Subtract(x1);
            //T dy = y2.Subtract(y1);
            ...
            return CalcSqDistance(x1, y1, x2, y2).Sqrt();
        }

        //--------------------------------------------------------calc_sq_distance
        public static T CalcSqDistance<T>(T x1, T y1, T x2, T y2)
             where T : IEquatable<T>, IComparable<T>, IComputable<T>, IConvertible, IFormattable
        {
            //double dx = x2 - x1;
            //double dy = y2 - y1;
        ...
        public static T CalcLinePointDistance<T>(T x1, T y1,
                                                   T x2, T y2,
                                                   T x, T y)
            where T : IEquatable<T>, IComparable<T>, IComputable<T>, IConvertible, IFormattable, ICommonNumericalOperations<T>, ITrigonometricOperations<T>
        {
            T dx = x2.Subtract(x1);
            T dy = y2.Subtract(y1);
            T d = dx.Multiply(dx).Add(dy.Multiply(dy)).Sqrt();
            if (d.LessThan(VertexDistEpsilon))
            {
                return CalcDistance(x1, y1, x, y);
            }
            return x.Subtract(x2).Multiply(dy).Subtract(y.Subtract(y2)).Multiply(dx).Divide(d);
        }

        //-------------------------------------------------------calc_line_point_u
        public static T CalcSegmentPoint<T>(T x1, T y1,
                                               T x2, T y2,
                                               T x, T y)
             where T : IEquatable<T>, IComparable<T>, IComputable<T>, IConvertible, IFormattable, ICommonNumericalOperations<T>, ITrigonometricOperations<T>
        {
            ...
            return pdx.Multiply(dx).Add(pdy.Multiply(dy).Divide(dx.Multiply(dx).Add(dy.Multiply(dy))));
        }

        //---------------------------------------------calc_line_point_sq_distance
        public static T CalcSegmentPointSqDistance<T>(T x1, T y1,
                                                         T x2, T y2,
                                                         T x, T y,
                                                         T u)
            where T : IEquatable<T>, IComparable<T>, IComputable<T>, IConvertible, IFormattable, ICommonNumericalOperations<T>, ITrigonometricOperations<T>
        {
            if (u.LessThanOrEqualTo(0))
            {
                ...
                if (u.GreaterThanOrEqualTo(1))
                {
                    return CalcSqDistance(x, y, x2, y2);
                }
            return CalcSqDistance(x, y, x1.Add(u).Multiply(x2.Subtract(x1)), y1.Add(u).Multiply(y2.Subtract(y1)));
        }
        ...
        {
            return
                CalcSegmentPointSqDistance(
                    x1, y1, x2, y2, x, y,
                    CalcSegmentPoint(x1, y1, x2, y2, x, y));
        }

        //-------------------------------------------------------calc_intersection
        public static bool CalcIntersection<T>(T ax, T ay, T bx, T by,
                                          T cx, T cy, T dx, T dy,
                                          out T x, out T y)
             where T : IEquatable<T>, IComparable<T>, IComputable<T>, IConvertible, IFormattable, ICommonNumericalOperations<T>, ITrigonometricOperations<T>
        {
            T num = ay.Subtract(cy).Multiply(dx.Subtract(cx)).Subtract(ax.Subtract(cx).Multiply(dy.Subtract(cy)));
            T den = bx.Subtract(ax).Multiply(dy.Subtract(cy)).Subtract(by.Subtract(ay).Multiply(dx.Subtract(cx)));
            if (den.Abs().LessThan(IntersectionEpsilon))
            {
                x = M.Zero<T>();
                y = M.Zero<T>();
                return false;
            ...
            T r = num.Divide(den);
            x = ax.Add(r.Multiply(bx.Subtract(ax)));
            y = ay.Add(r.Multiply(by.Subtract(ay)));
            return true;
        }
        ...
        //--------------------------------------------------------calc_orthogonal
        public static void CalcOrthogonal<T>(T thickness,
                                        T x1, T y1,
                                        T x2, T y2,
                                        out T x, out T y)
            where T : IEquatable<T>, IComparable<T>, IComputable<T>, IConvertible, IFormattable, ICommonNumericalOperations<T>, ITrigonometricOperations<T>
        {
            T dx = x2.Subtract(x1);
            T dy = y2.Subtract(y1);
            T d = M.Length(dx, dy);  //Math.Sqrt(dx * dx + dy * dy);
            x = thickness.Multiply(dy).Divide(d);
            y = thickness.Negative().Multiply(dx).Divide(d);
        }

        //--------------------------------------------------------dilate_triangle
        public static void DilateTriangle<T>(T x1, T y1,
                                        T x2, T y2,
                                        T x3, T y3,
                                        T[] x, T[] y,
                                        T d)
            where T : IEquatable<T>, IComparable<T>, IComputable<T>, IConvertible, IFormattable, ICommonNumericalOperations<T>, ITrigonometricOperations<T>
        {
            ...
            T dx3 = M.Zero<T>();
            T dy3 = M.Zero<T>();
            T loc = CrossProduct(x1, y1, x2, y2, x3, y3);
            if (loc.Abs().GreaterThan(IntersectionEpsilon))
            {
                if (CrossProduct(x1, y1, x2, y2, x3, y3).GreaterThan(0.0))
                {
                    d = d.Negative();
                }
                CalcOrthogonal(d, x1, y1, x2, y2, out dx1, out dy1);
                CalcOrthogonal(d, x2, y2, x3, y3, out dx2, out dy2);
                CalcOrthogonal(d, x3, y3, x1, y1, out dx3, out dy3);
            }
            x[0] = x1.Add(dx1); y[0] = y1.Add(dy1);
            x[1] = x2.Add(dx1); y[1] = y2.Add(dy1);
            x[2] = x2.Add(dx2); y[2] = y2.Add(dy2);
            x[3] = x3.Add(dx2); y[3] = y3.Add(dy2);
            x[4] = x3.Add(dx3); y[4] = y3.Add(dy3);
            x[5] = x1.Add(dx3); y[5] = y1.Add(dy3);
        }
        ...
        public static ushort[] GSqrtTable =                       //----------g_sqrt_table
        {
            0,
        ...
        public static byte[] GElderBitTable = //---------g_elder_bit_table
        {
        ...
        public static uint FastSqrt(uint val)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (bit != 0)
                    {
                        bit = GElderBitTable[bit] + 8;
                    }
                    else
                    {
                        bit = GElderBitTable[t];
                    }
                }
            }
            ...
            return (uint)((int)GSqrtTable[val] >> (int)shift);
        }
        ...
        public static T Besj<T>(T x, int n)
            where T : IEquatable<T>, IComparable<T>, IComputable<T>, IConvertible, IFormattable, ICommonNumericalOperations<T>, ITrigonometricOperations<T>
        {
            if (n < 0)
            {
            ...
            T d = M.New<T>(1E-6);
            T b = M.Zero<T>();
            if (x.Abs().LessThanOrEqualTo(d))
            {
                if (n != 0) return M.Zero<T>();
                return M.One<T>();
            }
            T b1 = M.Zero<T>(); // b1 is the Value from the previous iteration
            // Set up a starting order for recurrence
            int m1 = x.Abs().Add(6).ToInt();
            if (x.Abs().GreaterThan(5))
            {
                m1 = (int)M.New<T>(1.4).Multiply(x).Add(M.New<T>(60).Divide(x)).Abs().ToInt();
            }
            int m2 = (int)(M.New<T>(n).Add(2).Add(x.Abs().Divide(4)).ToInt());
            if (m1 > m2)
            {
                m2 = m1;
            }
            ...
            for (; ; )
            {
                ...
                for (int i = 1; i <= imax; i++)
                {
                    T c6t = M.New<T>(2).Multiply(M.New<T>(m2).Subtract(i)).Multiply(c2).Divide(x).Subtract(c3);
                    c3 = c2;
                    c2 = c6t;
                    if (m2 - i - 1 == n)
                    {
                    ...
                    if (m8 > 0)
                    {
                        c4 = c4.Add(M.New<T>(2).Multiply(c6t));
                    }
                }
                T c6 = M.New<T>(2).Multiply(c2).Divide(x).Subtract(c3);
                if (n == 0)
                {
                    b = c6;
                }
                c4.AddEquals(c6);
                b.DivideEquals(c4);
                if (b.Subtract(b1).Abs().LessThan(d))
                {
                    return b;
                }
                b1 = b;
