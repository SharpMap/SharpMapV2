...
namespace SharpMap.Web.Wms
{
	...
	public class WmsServer
	{
		...
		{
			if (map == null)
				throw new ArgumentNullException("map", "Map for WMS is null");

			if (map.Layers.Count == 0)
				throw new ArgumentException("Map doesn't contain any layers for WMS service");

			if (System.Web.HttpContext.Current == null)
                throw new InvalidOperationException("An attempt was made to access the WMS server outside a valid HttpContext");
		...
		private static SharpMap.Geometries.BoundingBox ParseBBOX(string strBBOX)
		{
			...
			if(strVals.Length!=4)
				return SharpMap.Geometries.BoundingBox.Empty;
