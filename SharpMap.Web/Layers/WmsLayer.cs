...
namespace SharpMap.Layers
{
	...
	public class WmsLayer : SharpMap.Layers.Layer
	{
		...
		public override void Render(IRenderSurface mapView, Map map)
		{
			...
			try
			{
				...
				{
                    System.IO.Stream imageStream = myWebResponse.GetResponseStream();
					if (_ImageAttributes != null)
                        mapView.DrawImage(imageStream, this.ImageAttributes);
					else
						mapView.DrawImage(img);
				}
		...
		public string GetRequestUrl(BoundingBox box, Size size)
		{
            ...
            if (_SpatialReferenceSystem == string.Empty)
				throw new InvalidOperationException("Spatial reference system not set");
