	...
	public class Ellipsoid : Info, IEllipsoid
	{
		...
		public override string Wkt
		{
			get
			{
                ...
                sb.AppendFormat(Info.NumberFormat, "SPHEROID[\"{0}\", {1}, {2}", Name, SemiMajorAxis, InverseFlattening);
		...
		public override string Xml
		{
			get
			{
                return String.Format(Info.NumberFormat,
					"<CS_Ellipsoid SemiMajorAxis=\"{0}\" SemiMinorAxis=\"{1}\" InverseFlattening=\"{2}\" " +
                        "IvfDefinitive=\"{3}\">{4}{5}</CS_Ellipsoid>",
