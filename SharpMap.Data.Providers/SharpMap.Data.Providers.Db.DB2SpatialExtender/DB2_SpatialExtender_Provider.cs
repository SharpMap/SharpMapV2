...
namespace SharpMap.Data.Providers
{
    ...
    public class DB2_SpatialExtender_Provider<TOid>
        : SpatialDbProviderBase<TOid>, ISpatialDbProvider<DB2_SpatialExtender_Utility>
    {

        #region Static Properties

        static DB2_SpatialExtender_Provider()
        {
            AddDerivedProperties(typeof(DB2_SpatialExtender_Provider<TOid>));
        }
        ...
        private Int32 _dataPageNumber = 0;
        private Int32 _dataPageSize = 0;
...
WHERE SRS.ORGANIZATION = {0} AND SRS.ORGANIZATION_COORDSYS_ID = {1};", SpatialReference.Authority, SpatialReference.AuthorityCode), cn).ExecuteReader();
                            if (dr.HasRows)
                            {
                                dr.Read();
   ...
   WHERE SRS.SRS_ID = {1};", 
                    ...
                    if (dr.HasRows)
                    {
                        ...
                        SharpMap.Utilities.GeometryServices gserv = new SharpMap.Utilities.GeometryServices();
                        if (!dr.IsDBNull(2))
                        {
                        ...
                        {
                            OriginalSpatialReference = new SharpMap.Utilities.GeometryServices()
        ...
        public String DB2SrsName
        {
            get
            {
                return _db2SrsId > -1 ?_db2SrsName : "DEFAULT_SRS";
            }
        }

        public DB2_SpatialExtender_Provider(IGeometryFactory geometryFactory, string connectionString, string tableName)
            : this(geometryFactory, connectionString, DB2_SpatialExtender_ProviderStatic.DefaultSpatialSchema, tableName, "OID", getGeometryColumnName(connectionString, DB2_SpatialExtender_ProviderStatic.DefaultSpatialSchema, tableName))
        {
        }

        private static String getGeometryColumnName(string connectionString, String schemaName, String tableName)
        {
            String columnName = DB2_SpatialExtender_ProviderStatic.DefaultGeometryColumnName;
            using (DB2Connection cn = new DB2Connection(connectionString))
            {
                cn.Open();
                try
...
SELECT COLS.COLUMN_NAME 
    ...
    WHERE (COLS.TABLE_SCHEMA = '{1}' AND COLS.TABLE_NAME = '{2}');",
        ...
        public DB2_SpatialExtender_Provider(IGeometryFactory geometryFactory, string connectionString,
                                  string tableSchema, string tableName, string oidColumn, string geometryColumn)
            : base(
                    new DB2_SpatialExtender_Utility(), geometryFactory, connectionString, tableSchema,
                    tableName,
                    oidColumn,
                    geometryColumn)
        {
    ...
    WHERE ({4}=MIN({4}));", 
        ...
        private OgcGeometryType parseGeometryType(String geometryString)
        {
            if (String.IsNullOrEmpty(geometryString))
                throw new DB2_SpatialExtender_Exception("Geometry type not specified!");

            switch (geometryString.ToUpper())
            {
                case "ST_GEOMETRY": return OgcGeometryType.Geometry;
                case "ST_POINT": return OgcGeometryType.Point;
                case "ST_LINESTRING": return OgcGeometryType.LineString;
                case "ST_POLYGON": return OgcGeometryType.Polygon;
                case "ST_MULTIPOINT": return OgcGeometryType.MultiPoint;
                case "ST_MULTILINESTRING": return OgcGeometryType.MultiLineString;
                case "ST_MULTIPOLYGON": return OgcGeometryType.MultiPolygon;
                case "ST_GEOMETRYCOLLECTION": return OgcGeometryType.GeometryCollection;
                default:
                    throw new DB2_SpatialExtender_Exception(string.Format("Invalid geometry type '{0}'", geometryString));
            }
        }
        ...
        public override string GeometryColumnConversionFormatString
        {
            get
            {
                return String.Format("{0}.ST_ASBINARY({1})",
                    DB2_SpatialExtender_ProviderStatic.DefaultSpatialSchema, "{0}");
            }
        }
        ...
        public override string GeomFromWkbFormatString
        {
            get
            {
                ...
                //"{0}", 
                //Srid == null ? DB2_SpatialExtender_ProviderStatic.DefaultSrid : Srid, 
                //TableSchema);
        ...
        public override string QualifiedTableName
        {
            get
            {
                if (String.IsNullOrEmpty(Table))
                    throw new DB2_SpatialExtender_Exception("Table name mustnot be null or emtpy");
        ...
        {
            return new DB2_SpatialExtender_ExpressionTreeToSqlCompiler<TOid>(this, expression);
        }
        ...
        protected override string InsertClause(IDbCommand cmd)
        {
            ...
            sets.Add(string.Format(GeomFromWkbFormatString, "@PGeo"));
                ...
                //        TableSchema, 
                //        DB2_SpatialExtender_ProviderStatic.ToDb2GeometryType(_validGeometryType))
                //)));
                                              ...
                                              ExpressionTreeToSqlCompilerBase<TOid> compiler)
        {
            ...
            string orderByCols = String.Join(",",
                ...
                sql = String.Format("\nSELECT {0}\nFROM {1}\n{2}\n{3} {4}\n{5};",
                                     mainQueryColumns,
                                     QualifiedTableName + " AS \"" + this.Table + "\"",
        ...
        protected override string GenerateSelectSql(IList<ProviderPropertyExpression> properties, ExpressionTreeToSqlCompilerBase<TOid> compiler, int pageSize, int pageNumber)
        {
            return "";
        ...
        protected override IFeatureDataReader ExecuteFeatureDataReader(IDbCommand cmd)
        {
            ...
            return new DB2_SpatialExtender_FeatureDataReader(GeometryFactory, dr, GeometryColumn, OidColumn) { CoordinateTransformation = CoordinateTransformation };

        }

        

        #region ISpatialDbProvider<DB2_SpatialExtender_Utility> Members

        public new DB2_SpatialExtender_Utility DbUtility
        {
            get { return (DB2_SpatialExtender_Utility)base.DbUtility; }
        }
