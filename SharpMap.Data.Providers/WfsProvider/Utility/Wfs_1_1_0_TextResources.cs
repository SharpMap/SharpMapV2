...
{
    public class WFS_1_1_0_TextResources : WFS_1_1_0_XPathTextResources, IWfsTextResources
    {
        ...
        public string GetFeatureGETRequest(WfsFeatureTypeInfo featureTypeInfo, IExtents boundingBox, IFilter filter)
        {
            ...
            filterBuilder.Append(featureTypeInfo.WfsGeometry.GeometryName);
            ...
            filterBuilder.Append(XmlConvert.ToString(boundingBox.GetMin(Ordinates.X)) + ",");
            filterBuilder.Append(XmlConvert.ToString(boundingBox.GetMin(Ordinates.Y)) + "%20");
            filterBuilder.Append(XmlConvert.ToString(boundingBox.GetMax(Ordinates.X)) + ",");
            filterBuilder.Append(XmlConvert.ToString(boundingBox.GetMax(Ordinates.Y)));
            ...
            return "?SERVICE=WFS&Version=1.1.0&REQUEST=GetFeature&TYPENAME=" + qualification + featureTypeInfo.Name +
                   "&PROPERTYNAME=" + qualification + featureTypeInfo.WfsGeometry.GeometryName +
        ...
        public byte[] GetFeaturePOSTRequest(WfsFeatureTypeInfo featureTypeInfo, string labelProperty,
                                            IExtents boundingBox, IFilter filter)
        {
            string qualification = string.IsNullOrEmpty(featureTypeInfo.Prefix)
                                       ? string.Empty
                                       : featureTypeInfo.Prefix + ":";
            ...
            using (StringWriter sWriter = new StringWriter())
            {
                using (XmlTextWriter xWriter = new XmlTextWriter(sWriter))
                {
                    ...
                    xWriter.WriteElementString("PropertyName", qualification + featureTypeInfo.WfsGeometry.GeometryName);
                    ...
                    if (!string.IsNullOrEmpty(featureTypeInfo.Prefix) &&
                        ...
                        xWriter.WriteElementString("PropertyName",
                                                   qualification + featureTypeInfo.WfsGeometry.GeometryName);
                            //added qualification to get it to work for deegree default sample
                    else
                        xWriter.WriteElementString("PropertyName", featureTypeInfo.WfsGeometry.GeometryName);
                    ...
                    xWriter.WriteElementString("lowerCorner", NSGML,
                                               XmlConvert.ToString(boundingBox.GetMin(Ordinates.X)) + " " +
                                               XmlConvert.ToString(boundingBox.GetMin(Ordinates.Y)));
                    xWriter.WriteElementString("upperCorner", NSGML,
                                               XmlConvert.ToString(boundingBox.GetMax(Ordinates.X)) + " " +
                                               XmlConvert.ToString(boundingBox.GetMax(Ordinates.Y)));
